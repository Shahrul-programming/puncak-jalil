...
@section('title', 'Tambah Rider Baru')
    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex items-center mb-6">
                ...
                <h1 class="text-2xl font-bold text-gray-900">Tambah Rider Baru</h1>
            </div>
            ...
            <form method="POST" action="{{ route('riders.store') }}" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="vehicle_type" name="vehicle_type"
                            ...
                            <option value="motorcycle" {{ old('vehicle_type') == 'motorcycle' ? 'selected' : '' }}>Motosikal</option>
                            <option value="car" {{ old('vehicle_type') == 'car' ? 'selected' : '' }}>Kereta</option>
                            <option value="bicycle" {{ old('vehicle_type') == 'bicycle' ? 'selected' : '' }}>Basikal</option>
                            <option value="walking" {{ old('vehicle_type') == 'walking' ? 'selected' : '' }}>Berjalan</option>
                        </select>
                        @error('vehicle_type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="address" name="address" rows="3"
                                  ...
                                  placeholder="Masukkan alamat lengkap rider" required>{{ old('address') }}</textarea>
                        @error('address')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    ...
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md font-medium">
                        <i class="fas fa-save mr-2"></i>Simpan Rider
                    </button>
                </div>
            </form>
        </div>
