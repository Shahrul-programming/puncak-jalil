...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <p class="text-gray-600 dark:text-gray-400 mt-1">Kemaskini maklumat promosi anda</p>
        </div>

        <!-- Form -->
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md">
            <form method="POST" action="{{ route('promotions.update', $promotion->id) }}" enctype="multipart/form-data" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="shop_id" id="shop_id" 
                        ...
                        @foreach($shops as $shop)
                            <option value="{{ $shop->id }}" {{ (old('shop_id', $promotion->shop_id) == $shop->id) ? 'selected' : '' }}>
                                {{ $shop->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4"
                              ...
                              placeholder="Terangkan promosi anda dengan lebih terperinci...">{{ old('description', $promotion->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                        Format yang disokong: JPG, PNG, GIF. Maksimum 2MB. Biarkan kosong jika tidak ingin mengubah gambar.
                    </p>
                    @error('image')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="status" id="status" 
                        ...
                        <option value="active" {{ old('status', $promotion->status) == 'active' ? 'selected' : '' }}>Aktif</option>
                        <option value="inactive" {{ old('status', $promotion->status) == 'inactive' ? 'selected' : '' }}>Tidak Aktif</option>
                    </select>
                    @error('status')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex justify-end space-x-4 pt-6 border-t border-gray-200 dark:border-gray-600">
                    ...
                    <button type="submit" 
                            class="px-6 py-2 bg-yellow-600 hover:bg-yellow-700 text-white rounded-md font-medium transition duration-200">
                        <i class="fas fa-save mr-2"></i>Kemaskini Promosi
                    </button>
                </div>
            </form>
        </div>
